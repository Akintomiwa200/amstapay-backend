...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    await Promise.all([
      Wallet.findOneAndDelete({ user: req.user._id }),
      Transaction.deleteMany({ $or: [{ sender: req.user._id }, { receiver: req.user._id }] }),
      DeviceSession.deleteMany({ user: req.user._id }),
    ...
    const { response } = createResponse(true, "Account and all associated data deleted successfully");
    res.json(response);
  } catch (err) {
